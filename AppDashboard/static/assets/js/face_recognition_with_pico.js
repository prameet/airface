document.addEventListener('DOMContentLoaded', function () {
	...
	// canvas
	let video_canvas = document.getElementById('canvasOutput');
	...
	const video_constraints_for_rear = { video: { facingMode: "environment" } };
	...
	*/
	...
	const cascadeurl = 'static/facefinder';
	...
	*/
	...
	// var baseURL = "http://65.0.51.114:5003/";
	var baseURL = sessionStorage.getItem("baseURL");
	var prefix = "api/v1/";
	var companyMail = sessionStorage.getItem('OrganizationEmailId');
	var companyNameData = sessionStorage.getItem('OrganizationName');
	var EnrollEmpid = sessionStorage.getItem('EnrollEmpid');
	var enroll_endpoint = 'enroll/' + EnrollEmpid;
	var enrollprofileget = baseURL + prefix + enroll_endpoint;
	var enrollsettings = {
		"url": enrollprofileget,
		"async": true,
		"crossDomain": true,
		"method": "GET",
		"headers": {
	...
	$.ajax(enrollsettings).done(function (infodata) {
		console.log(infodata);
		var info = infodata['ResponseData'];
		Counter = info.length;
		console.log(Counter);
		for (i = 0; i < info.length; i++) {
			Counter++;
			var clonedDiv = $('#img').clone(true);
			clonedDiv.prop('id', 'defaultCanvas' + Counter);
			clonedDiv.appendTo("#SamplesCollected");
			$('#defaultCanvas' + Counter).show();
			var imgData = baseURL + info[i]['ImagePath'];
			$('#defaultCanvas' + Counter).find('img').attr("src", imgData);
			$('#defaultCanvas' + Counter).find('img').attr("width", '100%;');
			$("[id='defaultCanvas" + Counter + "'] .dltbtnadd a").attr('onClick', 'deleteCaptureURL(' + Counter + ',' + info[i]['DatasetEncodingsId'] + ');');
			$('#defaultCanvas' + Counter).find('input').attr("value", imgData);
			var resImg = $("[id^='defaultCanvas']").length;
			$('#cStatus').text(resImg + '/12');
		}
	...
	captureBtn.addEventListener('click', () => {
		...
		//alert('newwb second');
		if ($("[id^='defaultCanvas']").length < 12) {

			console.log("clicked" + Counter);


			const actionBtn = document.getElementById('actionBtn');
			const canvasOrg = document.querySelector('.cliped-img-1');
			const width = 640;
			const height = 480;
			if (canvasOrg) {
				Counter++;
				...
				clonedDiv.appendTo("#SamplesCollected");
				$('#defaultCanvas' + Counter).show();
				var imgData = canvasOrg.toDataURL('image/png');
				$('#defaultCanvas' + Counter).find('img').attr("src", imgData);
				// $('#defaultCanvas'+ Counter).find('img').attr("width",'50px;');
				$('#defaultCanvas' + Counter).find('img').attr("width", '100%;');
				var deleteBtn = "<input type='hidden' id='multiFiles[]' name='fileToUpload[]'>";
				$('#defaultCanvas' + Counter).append(deleteBtn);
				$("[id='defaultCanvas" + Counter + "'] .dltbtnadd a").attr('onClick', 'deleteCapture(' + Counter + ');');
				$('#defaultCanvas' + Counter).find('input').attr("value", imgData);
				var resImg = $("[id^='defaultCanvas']").length;
				$('#cStatus').text(resImg + '/12');

				// call enroll service here

				var StorageEmpId = sessionStorage.getItem("EnrollEmpid");
				var StorageEmpName = sessionStorage.getItem("EmpName");

				var StorageEnrollbLoc = sessionStorage.getItem("EnrollbLoc");
				var StorageAccessToken = sessionStorage.getItem("AccessToken");
				var StorageLicKey = sessionStorage.getItem("LicKey");
				var ImageString = imgData;

				var settings = {
					"url": "http://localhost:5018/api/v1/enroll/1",
					"method": "POST",
					"timeout": 0,
					"headers": {
						"Content-Type": "application/json",
						"LicKey": StorageLicKey,
						"AccessToken": StorageAccessToken
					},
					"data": JSON.stringify({
						"EmpId": StorageEmpId,
						"EmpName": StorageEmpName,
						"BaseLocationId": StorageEnrollbLoc,
						"ImagePath": [
							ImageString
						]
					}),
				...
				console.log("You can add max 12 photos");
				//alert("You can add max 12 photos");
				//$('#msgshow').text('You can add max 12 photos.').show();
				//$('#msgshow').show();
			}
		}
		else {
			// alert("Sorry ! you can add max 12 photos");
			$('#msgerror').text('Sorry ! you can send max 12 photos for enroll.').show();
			$('#msgshow').show();
		}
	});
...
//For : Delete Current Capture Image
function deleteCapture(count) {
	$('#defaultCanvas' + count).remove();
	var resImg = $("[id^='defaultCanvas']").length;
	$('#cStatus').text(resImg + '/12');

}

//For : Delete Past Capture Image By URL
function deleteCaptureURL(count, ID) {
	if (confirm("Are you sure you want to Delete this Profile?")) {
		var endpoint = 'enroll/' + ID;
		...
		$.ajax(deleteImg).done(function (response) {
...
//For : Clear All Image by one click
function clearThumbs() {
	document.getElementById("msgshow").style.display = "none";
	$(".thumbs").not($("[id='img']")).remove();
	var resImg = $("[id^='defaultCanvas']").length;
	$('#cStatus').text(resImg + '/12');
}  
